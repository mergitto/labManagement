    ...
    public function index()
    {
        ...
        foreach ($events as $key => $event) {
          ...
          foreach ($event->users as $key => $eventUser) {
            if(isset($checkUsers[$event->id])){
              // ファイルを登録しているユーザーがいるかどうか && ファイルを登録したユーザーが割り当てられているかどうか
              if(!is_null($checkUsers[$event->id]) && array_search($eventUser->id, $checkUsers[$event->id]) !== FALSE) {
